...
|---|---|---|---|
| FIX-1 calibrated threshold, never a fixed `0.5` | `src/calibration.ts` | `tests/calibration.test.ts` "calibration and minimum retention", `tests/parity.test.ts` "cross-language calibration outcomes match the Python golden" | Passing |
| FIX-2 assistant-text anchors protected verbatim | `src/anchors.ts`, `src/prepass.ts`, `src/store.ts` hint index and assembly | `tests/anchors.test.ts` "assistant spans are exact", `tests/lcm-vertical.test.ts` "LCM active context assembles protected verbatim evidence before summaries within budget", "protected keep is exact, while truncate uses only configured head and tiny budgets keep pointers" | Passing |
...
| FIX-4 tiered shrink ladder, hard cap, explicit `jev_unscored` | `src/state-shaper.ts` | `tests/batcher.test.ts` "541 candidates: batched, bounded, unscored and recoverable", `tests/state-shaper.test.ts` "CJK state cap preserves candidates or marks them unscored" | Passing |
| FIX-5 batched scoring across turns | `src/batcher.ts`, `src/prepass.ts` serialized flush queue | `tests/batcher.test.ts` "541 candidates: batched, bounded, unscored and recoverable", `tests/concurrency.test.ts` "overlapping forced flushes score each candidate once", "summary persistence failure prevents returning a replacement" | Passing |
| FIX-6 honest metrics and low-freed warning | `src/metrics.ts` | `tests/jev-client.test.ts` "malformed payloads and unsafe endpoints are rejected", `tests/metrics.test.ts` "low freed warnings are emitted once per three cycles" | Passing |
| Dual-provider authentication with identical resolution and fallback behavior | `src/providers.ts`, `src/jev-client.ts`, `src/settings.ts` | `tests/providers.test.ts` "provider resolution, fallback, cooldown and no secret logging", `tests/fallback.test.ts` "fallback fires once per provider and expires with the cooldown", `tests/parity.test.ts` "cross-language provider selection and fallback outcomes match the Python golden" | Passing |
...
| Recall-at-budget evaluator, three arms | `evaluation/run_eval.ts` | `tests/evaluation.test.ts` "all three arms run through real code and the shipped arm converges", "retention responds to actual model transport content", "invalid budgets and unknown arms are rejected", "the bundled fixture is complete and names its own limits" | Passing |
| OpenRouter surface selection, native Decisions by default with a Decisions shaped chat completions adapter | `src/settings.ts` (`openrouter_endpoint_path`), `src/providers.ts` (`chatRequest`, `decisionsAnswers`) | `tests/providers.test.ts` "OpenRouter chat adapter matches the native decisions surface", "OpenRouter chat adapter sends a chat body carrying the same questions", "OpenRouter chat adapter rejects malformed completions", "OpenRouter chat path passes a decisions shaped gateway response through" | Passing |
| Calibration and provider diagnostics, with a per-provider dry run probe | `src/index.ts` (`jev_calibrate`), `src/providers.ts` (`probeProviders`) | `tests/providers.test.ts` "dry run probe reports latency and status per provider" | Passing |
...
|---|---|---|
| Uniform low `keep_result` probabilities still keep at least 10% of candidates | `tests/calibration.test.ts` "calibration and minimum retention" uses `i/2500` over 500 samples and asserts at least 50 retained | Passing |
...
| A 541-call transcript yields unscored candidates that stay retrievable | `tests/batcher.test.ts` "541 candidates: batched, bounded, unscored and recoverable" expands each unscored candidate and greps it | Passing |
| Three consecutive low-freed cycles emit a warning | `tests/jev-client.test.ts` "malformed payloads and unsafe endpoints are rejected", `tests/metrics.test.ts` "low freed warnings are emitted once per three cycles" asserts exactly one emitted warning | Passing |
| Two consecutive turns inside the batch window do not produce two Jev requests | `tests/batcher.test.ts` "541 candidates: batched, bounded, unscored and recoverable" asserts zero calls after two ticks and one at the window boundary | Passing |
| OpenRouter alone loads and scores | `tests/providers.test.ts` "provider resolution, fallback, cooldown and no secret logging", `tests/fallback.test.ts` "fallback fires once per provider and expires with the cooldown" scores through an OpenRouter-only chain | Passing |
...
| Both keys missing disables scoring while LCM condensation proceeds without raising | `tests/fallback.test.ts` "missing provider keys disable ranking while LCM condensation proceeds" | Passing |
| No key value appears in a log line, metric, or error | `tests/providers.test.ts` "provider resolution, fallback, cooldown and no secret logging", `tests/fallback.test.ts` "fallback fires once per provider and expires with the cooldown" asserts no `SECRET_` value in logs or diagnostics | Passing |
