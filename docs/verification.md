...
- 37 tests pass. `pnpm run typecheck` (source and tests, the latter through `tsconfig.test.json`) and `pnpm run build` pass. `pnpm pack` builds the `@bojansandhaus/jev-lcm-dsh-compaction` 1.0.0-rc.1 archive.
