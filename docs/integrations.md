...
The third route is the explicit opt-in `laya_then_hosted`, which the DOGA fork names `laya_with_jev_fallback`; `laya_local` is accepted as the alias for plain `laya`, and `settings()` resolves either alias to the canonical value so no alias reaches a chain, a diagnostic, or a log. The local server leads and the hosted providers follow, as `laya` plus the members of `jev_fallback_order` that have a key. The existing triggers, cooldown, and retries apply unchanged, so a transport error, timeout, `401`, `403`, `429`, or `5xx` from the local server falls through to the hosted hop. Selecting the mode with no hosted key at all fails at load and names the missing environment variables, because the promised fallback cannot exist. The remote hop is bounded: three consecutive local failures still fall through, and every further failure until a local success suppresses the hosted leg, logs `laya_fallback_suppressed count=<n> limit=3 reason=<category>`, and re-raises the local error. The count is per process and resets on restart, so it bounds repeated remote egress rather than acting as a durable rate limit, and it cannot detect a valid yet incorrect local judgment. Privacy consequence: unlike plain `laya`, this route can send state to a hosted API, since a failed local attempt re-sends it there. Coverage is `tests/laya-then-hosted.test.ts` for the chain and `tests/laya-fallback-breaker.test.ts` for the breaker, and its hosted leg is exercised with an injected synthetic transport only. The DOGA fork's matched 100-question evaluation of the same local classifier is the headline quality evidence and is summarised in [reference.md](reference.md) and the README.
